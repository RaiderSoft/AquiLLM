...
class IngestMonitorConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        if is_authenticated:
            ...
            await self.close()
        await self.channel_layer.group_add(f"document-ingest-{self.scope['url_route']['kwargs']['doc_id']}", self.channel_name)
...
class IngestionDashboardConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if is_authenticated:
            ...
            await self.close()
        await self.channel_layer.group_add(f"ingestion-dashboard-{self.user.id}", self.channel_name)
